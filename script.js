document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    function getErrorMessage(field, validationType) {
      const label = field.closest('.form-group')
        ? field.closest('.form-group').querySelector('label')
        : null;
      const fieldName = label ? label.textContent.trim() : (field.placeholder || 'This field');
    ...
    function clearFieldState(field) {
      field.classList.remove(FIELD_ERROR_CLASS, FIELD_SUCCESS_CLASS);
      const parent = field.parentElement;
      const existingError = parent.querySelector('.' + ERROR_MESSAGE_CLASS);
      if (existingError) {
    ...
    function setFieldSuccess(field) {
      clearFieldState(field);
      field.classList.add(FIELD_SUCCESS_CLASS);
    }
    ...
    function validateField(field) {
      ...
      if (isRequired && !validators.required(value)) {
        setFieldError(field, getErrorMessage(field, 'required'));
        return false;
      }
      ...
      if (type === 'email' && !validators.email(value)) {
        setFieldError(field, getErrorMessage(field, 'email'));
        return false;
      }

      if (type === 'tel' && !validators.phone(value)) {
        setFieldError(field, getErrorMessage(field, 'phone'));
        return false;
      }

      if (minLength && !validators.minLength(value, parseInt(minLength, 10))) {
        setFieldError(field, getErrorMessage(field, 'minLength'));
        return false;
      }
    ...
    function getFormFields() {
      return Array.from(
        contactForm.querySelectorAll('input:not([type="hidden"]):not([type="submit"]):not([type="button"]), textarea, select')
      );
    }
    ...
    contactForm.addEventListener('input', function (e) {
      const field = e.target.closest('input, textarea, select');
      if (!field || !contactForm.contains(field)) return;
      if (field.classList.contains(FIELD_ERROR_CLASS)) {
        validateField(field);
      }
    });
    ...
    contactForm.addEventListener('submit', function (e) {
      ...
      if (!isFormValid) {
        ...
        const firstError = contactForm.querySelector('.' + FIELD_ERROR_CLASS);
        if (firstError) {
  ...
  function applyStaggerDelay(entries) {
    ...
    visibleEntries.forEach(function (entry, index) {
      ...
      const baseDelay = parseFloat(element.getAttribute(STAGGER_ATTR) || 0);
      const staggerDelay = baseDelay + index * 0.1;
  ...
  if ('IntersectionObserver' in window) {
    ...
    function observeElements() {
      const elements = document.querySelectorAll('.' + OBSERVE_CLASS);
    ...
    const mutationObserver = new MutationObserver(function (mutations) {
      mutations.forEach(function (mutation) {
        mutation.addedNodes.forEach(function (node) {
          ...
          if (node.classList && node.classList.contains(OBSERVE_CLASS)) {
            scrollObserver.observe(node);
          }

          const children = node.querySelectorAll && node.querySelectorAll('.' + OBSERVE_CLASS);
          if (children) {
    ...
    const fallbackElements = document.querySelectorAll('.' + OBSERVE_CLASS);
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: 'b2e2d45a-89e5-4ff6-a1cc-e5c4729b8d41',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || 'b2e2d45a-89e5-4ff6-a1cc-e5c4729b8d41';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = 'b2e2d45a-89e5-4ff6-a1cc-e5c4729b8d41';
